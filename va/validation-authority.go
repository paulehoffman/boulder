...
func (va ValidationAuthorityImpl) getAddr(ctx context.Context, hostname string) (addr net.IP, addrs []net.IP, problem *probs.ProblemDetails) {
...
func (va *ValidationAuthorityImpl) validateChallengeAndCAA(ctx context.Context, identifier core.AcmeIdentifier, challenge core.Challenge, regID int64) ([]core.ValidationRecord, *probs.ProblemDetails) {
	...
	select {
	case <-ctx.Done():
		// TODO(#1292): stat and log
		return validationRecords, probs.ServerInternal("Unable to receive validation data in time")
	case caaProblem := <-ch:
		if caaProblem != nil {
